...
const sessionSchema = z.object({
  ...
  browserHost: z.string().trim().nullable().optional(),
  browserPath: z.string().trim().nullable().optional(),
  browserSearchQuery: z.string().trim().nullable().optional(),
  browserSurfaceType: z.string().trim().nullable().optional(),
  ...
  for (const session of parsed.data.sessions) {
    ...
    const tagsJson = JSON.stringify(
      autoTag({
        ...
        browserSurfaceType: session.browserSurfaceType ?? null,
      })
    );
    ...
    if (existing) {
      await db
        ...
        .set({
          ...
          browserHost: session.browserHost ?? null,
          browserPath: session.browserPath ?? null,
          browserSearchQuery: session.browserSearchQuery ?? null,
          browserSurfaceType: session.browserSurfaceType ?? null,
